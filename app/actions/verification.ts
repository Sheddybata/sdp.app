...
export async function verifyByMembershipId(membershipId: string): Promise<VerifyResult> {
  try {
    const trimmed = membershipId.trim();
    if (!trimmed) {
      return { ok: false, error: "Please enter your membership registration number." };
    }

    if (trimmed.replace(/\s/g, "").length < 6) {
      return {
        ok: false,
        error: "That number looks too short. Check the ID on your membership card and try again.",
      };
    }
    ...
    if (!member) {
      return {
        ok: false,
        error:
          "No record matches that membership registration number. Check for typos (use the exact ID on your card) and try again.",
      };
    }
...
export async function verifyByVoterId(voterIdInput: string): Promise<VerifyResult> {
  try {
    ...
    return {
      ok: false,
      error:
        "Member not found. Please check the voter registration number and try again. Make sure there are no spaces or extra characters.",
    };
  } catch (error) {
